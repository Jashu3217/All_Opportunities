...
@Component({
    ...
    <div class="module-page">
      ...
      <div class="module-header">
        <div class="module-title">
          ...
          <div>
            ...
            <p>{{ total() }} roles · {{ state.locationLabel() }} · Direct company apply links</p>
          </div>
        </div>
      ...
      <div *ngFor="let job of filteredJobs(); let i = index"
        ...
        <div class="card-header">
          <div class="card-left">
            <div class="card-title-row">
              <span class="live-dot" [style.background]="getRoleColor(job.roleType)" [style.box-shadow]="'0 0 6px ' + getRoleColor(job.roleType)"></span>
          ...
          <div class="score-bar-wrap">
            <div class="score-bar-track">
              <div class="score-bar-fill" [style.width]="job.score + '%'" [style.background]="getRoleColor(job.roleType)"></div>
            </div>
        ...
        <div class="platform-pills">
          <a *ngFor="let entry of getPlatformEntries(job.searchUrls)"
             [href]="entry.url" target="_blank" rel="noopener" class="platform-pill">
            {{ entry.label }}
          </a>
        </div>
      </div>
  ...
  styles: [`
    :host { display: block; }
    .module-page { }
    .module-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:18px; gap:12px; flex-wrap:wrap; }
    ...
    .card-meta { display:flex; gap:12px; font-size:11px; color:#4a7098; flex-wrap:wrap; }
    ...
    .company-pill { display:inline-flex; align-items:center; gap:5px; background:rgba(var(--cc,0,212,255),0.1); border:1px solid rgba(var(--cc,0,212,255),0.3); color:var(--cc,#00d4ff); font-size:11px; font-weight:700; padding:5px 11px; border-radius:8px; text-decoration:none; transition:all 0.15s; }
    ...
    .platform-pill { font-size:11px; font-weight:700; color:#00d4ff; background:rgba(0,212,255,0.1); border:1px solid rgba(0,212,255,0.3); padding:4px 10px; border-radius:7px; text-decoration:none; transition:background 0.15s; }
    .platform-pill:hover { background:rgba(0,212,255,0.2); }
...
export class SdeComponent implements OnInit {
  ...
  filteredJobs = computed(() =>
    this.jobs().filter(j => {
      const sc = this.scoreFilter === 'top' ? j.score >= 90
               : this.scoreFilter === 'good' ? j.score >= 80 && j.score < 90 : true;
      const rl = this.roleFilter === 'all' ? true : j.roleType === this.roleFilter;
      return sc && rl;
    })
  );
  ...
  load() {
    ...
    this.api.getSdeJobs(this.state.location()).subscribe({
  ...
  getRoleColor(role: string): string {
    return role === 'SDE' ? '#00d4ff' : role === 'SWE' ? '#7c4dff' : '#00e676';
  }
  getIvColor(iv: string): string {
    const m: Record<string,string> = { 'DSA Heavy':'#ffb800','System Design':'#7c4dff','Both DSA + SD':'#00e676','Mostly SD':'#00d4ff' };
    return m[iv] || '#8aafd4';
  }
  getIvIcon(iv: string): string {
    const m: Record<string,string> = { 'DSA Heavy':'🧠','System Design':'🏗️','Both DSA + SD':'⚡','Mostly SD':'📐' };
    return m[iv] || '💼';
  }
  getPlatformEntries(urls: Record<string,string>): { label: string; url: string }[] {
    const labels: Record<string,string> = { naukri:'🔵 Naukri', linkedin:'💼 LinkedIn', indeed:'🔍 Indeed', cutshort:'✂️ Cutshort', wellfound:'🚀 Wellfound', instahyre:'⚡ Instahyre' };
    return Object.entries(urls).filter(([k]) => k in labels).map(([k,u]) => ({ label: labels[k], url: u }));
  }
}
